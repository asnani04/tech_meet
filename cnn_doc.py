...
print("Loading data...")
x_train, x_test, y_train, y_test, sentences_padded_train, vocabulary, vocabulary_inv = get_data.load_data_and_labels(50000)
...
fc1_weights = tf.Variable(  #fully connected, depth 512.
    tf.truncated_normal(
        ...
        stddev=0.01))
fc1_biases = tf.Variable(tf.constant(0.01, shape=[num_classes]))
...
logits = tf.matmul(pooled, fc1_weights) + fc1_biases
...
with tf.Session() as sess:
	...
	if os.path.exists("./cnn_doc_10000_2.ckpt"): # if Weights have been used
		...
		for step in xrange(int(no_of_epochs * train_size) // batch_size):
		...
		saver_weights.save(sess, "./cnn_doc_10000_3.ckpt")
		test_error = error_rate(eval_in_batches(x_test, sess), y_test)
		print('Test error: %.1f%%' % test_error)
		# saver_weights.save(sess, "./cnn_doc_10000.ckpt")
